...
class DatabaseCallBack @Inject constructor(
    ...
    private suspend fun populateDatabase(technologyDao: TechnologyDao, difficultyDao: DifficultyDao) {
        ...
        difficultyDao.insert(DifficultyEntity(id = 0, name = "Fácil", description = ""))
        difficultyDao.insert(DifficultyEntity(id = 1, name = "Intermedio", description = ""))
        difficultyDao.insert(DifficultyEntity(id = 2, name = "Difícil", description = ""))

        // Insertar tecnologías
        technologyDao.insert(TechnologyEntity(id = 0, name = "React", icon = 1, description = ""))
    }
}
